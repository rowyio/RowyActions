...
var validateAction = function (_a) {
    ...
    var _b = schemaSnapshot.get("columns.".concat(column.key, ".config")), requiredRoles = _b.requiredRoles, requiredFields = _b.requiredFields;
    ...
    if (missingRequiredFields.length > 0) {
        throw new Error("".concat(missingRequiredFields.length === 1
            ? "".concat(missingRequiredFields[0], " field is")
            : "".concat(missingRequiredFields.join(", "), " fields are"), " required and missing from this row"));
    }
    return true;
};
